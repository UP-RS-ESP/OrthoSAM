...
#prep
OutDIR='/DATA/vito/output/colour_test_single_samenoise2/'
...
stds=[0,10,25,40]
#run test
for run in np.arange(0,len(color_pair)):#range(len(color_pair)):
    inner=True#
    #inner=False
    var=True#
    #var=False
    ...
    np.save(OutDIR+f'collect_test.npy',test_outputs)
channel_s=['R','G','B']

max_iou=[out['All_max_iou'] for out in test_outputs]

for channel in range(3):
    ticks=range(len(color_spacing))
    labels=(color_spacing*255).astype(int)
    ...
    for i in range(4):#loop through radi
        ...
        for j in range(4):#loop through noise level
            ...
            plt.title(f'Circle radius: {radi[i]}\nGaussian noise: {stds[j]}')
            plt.imshow(np.vstack([arg[:,col][channel] for arg in max_iou]).reshape((len(color_spacing),len(color_spacing))), origin='lower'#, aspect='auto')
